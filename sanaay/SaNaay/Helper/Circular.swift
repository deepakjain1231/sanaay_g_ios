...
open class Circular: UIView {
    ...
    override public func draw(_ rect: CGRect) {
        ...
        let maxNumber = _percentages.max()

        for i in 0..<_percentages.count {

            let is_max = _percentages[i] == maxNumber ? true : false
            ...
            showPercentages(midAngel:startAngle + (endAngle - startAngle)/2, percentage: _percentages[i], is_max: is_max)
    ...
    private func showPercentages(midAngel:Double, percentage:Double, is_max: Bool = false) {
        ...
        if is_max {
            percentageLabel.textColor = #colorLiteral(red: 0.9647058824, green: 0.1568627451, blue: 0.0431372549, alpha: 1)
        }
        else {
            percentageLabel.textColor = #colorLiteral(red: 0.06666666667, green: 0.06666666667, blue: 0.06666666667, alpha: 1)
        }
